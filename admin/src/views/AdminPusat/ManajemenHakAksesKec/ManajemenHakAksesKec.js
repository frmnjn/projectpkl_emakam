...
class ManajemenHakAksesKec extends Component {
  ...
  fetchall() {
    ...
    fetch('http://149.28.138.217/api/kecamatan/constraint_user?token=' + sessionStorage.getItem('token'))
      .then(response => response.json())
      .then(
        (result) => {
          // result = result.filter(function (item) {
  ...
  handleSubmitCreate = event => {
    ...
    fetch('http://149.28.138.217/api/kecamatan/create_role?token=' + sessionStorage.getItem('token'), {
      method: 'POST',
      headers: {
  ...
  handleSubmitEdit = event => {
      ...
      fetch('http://149.28.138.217/api/kecamatan/update_role/' + this.state.active_id_role + "?token=" + sessionStorage.getItem('token'), {
        method: 'PUT',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
  ...
  handledelete(table_constraint_user) {
    ...
    fetch('http://149.28.138.217/api/kecamatan/delete_role/' + table_constraint_user.id_role + "?token=" + sessionStorage.getItem('token'), {
      method: 'DELETE'
    })
      .then(
        this.fetchall
