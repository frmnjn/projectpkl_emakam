...
class CetakDokumen extends Component {
  ...
  componentDidMount() {
    fetch("http://178.128.81.243:8000/api/dokumen/view_siap_cetak?token=" + sessionStorage.getItem('token')+'&id_user='+sessionStorage.getItem('id_user'))
      .then(response => {
  ...
  cetak_dokumen = (items) => {
    ...
    if(items.no_surat_perizinan!=null){
      const url = 'http://178.128.81.243:8000/api/dokumen/cetak_surat_perizinan?token=' + sessionStorage.getItem('token')
